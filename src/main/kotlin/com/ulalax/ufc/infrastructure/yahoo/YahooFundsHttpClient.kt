...
/**
 * Yahoo Finance Funds HTTP 클라이언트 구현체
 *
 * 책임:
 * - HTTP 요청/응답 처리만 담당
 ...
 * - 외부 API 의존성 격리
 * - Domain은 FundsHttpClient 인터페이스만 의존
 ...
 */
internal class YahooFundsHttpClient(
    ...
    companion object {
        private val logger = LoggerFactory.getLogger(YahooFundsHttpClient::class.java)
    ...
    override suspend fun fetchQuoteSummary(
        ...
        return try {
            logger.debug(
                "Calling Yahoo Finance QuoteSummary API for funds: symbol={}, modules={}",
                symbol, modules
            )
            ...
            if (!response.status.isSuccess()) {
                throw ApiException(
                    message = "QuoteSummary API 요청 실패 (Funds): HTTP ${response.status.value}",
            ...
            logger.debug("QuoteSummary API response received (Funds): length={}", responseBody.length)
            ...
            try {
                ...
                if (quoteSummaryResponse.quoteSummary.error != null) {
                    throw ApiException(
                        message = "QuoteSummary API 에러 (Funds): ${quoteSummaryResponse.quoteSummary.error?.description ?: "Unknown error"}",
                        statusCode = 200
                    )
                }
                ...
                if (quoteSummaryResponse.quoteSummary.result.isNullOrEmpty()) {
                    throw UfcException(
                        ...
                        message = "QuoteSummary 데이터를 찾을 수 없습니다 (Funds): $symbol"
                    )
                }
                ...
                when (e) {
                    is UfcException, is ApiException -> throw e
                        ...
                        logger.error("JSON 파싱 실패 (Funds). 응답 내용: {}", responseBody.take(500), e)
                        throw DataParsingException(
                            message = "QuoteSummary 응답 JSON 파싱 실패 (Funds)",
            ...
            logger.error("Unexpected error while calling QuoteSummary API (Funds)", e)
            throw UfcException(
                errorCode = ErrorCode.EXTERNAL_API_ERROR,
                message = "QuoteSummary API 호출 중 오류 발생 (Funds): ${e.message}",
                cause = e
            )
        }
    }
