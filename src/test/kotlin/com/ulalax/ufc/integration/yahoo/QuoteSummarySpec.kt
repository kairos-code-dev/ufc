...
class QuoteSummarySpec : IntegrationTestBase() {
    ...
    inner class ResponseSpec {
        ...
        fun `requested modules are included in response`() = integrationTest {
            // Given
            val symbol = TestFixtures.Symbols.AAPL
        ...
        fun `multiple requested modules are all included`() = integrationTest {
            // Given
            val symbol = TestFixtures.Symbols.AAPL

            // When
            val result = ufc.yahoo.quoteSummary(
                symbol,
    ...
    inner class UsageExamples {
        ...
        fun `can fetch stock key information at once`() = integrationTest {
            // Given
    ...
    inner class AvailableModules {
        ...
        fun `has 36 modules defined`() = integrationTest {
            // Given
        ...
        fun `can identify major module categories`() = integrationTest {
            // 가격/시세 모듈
            val priceModules = listOf(
                QuoteSummaryModule.PRICE,
                QuoteSummaryModule.SUMMARY_DETAIL
            )

            // 기업 정보 모듈
            val companyInfoModules = listOf(
                QuoteSummaryModule.ASSET_PROFILE,
                QuoteSummaryModule.QUOTE_TYPE
            )

            // 재무 모듈
            val financialModules = listOf(
                QuoteSummaryModule.FINANCIAL_DATA,
                QuoteSummaryModule.DEFAULT_KEY_STATISTICS
            )

            // 실적 모듈
            val earningsModules = listOf(
    ...
    inner class ModuleCombinations {
        ...
        fun `can request single module`() = integrationTest {
            // Given
            val symbol = TestFixtures.Symbols.AAPL
        ...
        fun `can use stockModules preset`() = integrationTest {
            // Given
            val symbol = TestFixtures.Symbols.AAPL
            val stockModules = QuoteSummaryModule.stockModules()
    ...
    inner class ModuleDataSpec {
        ...
        inner class PriceModuleSpec {
            ...
            fun `can get current price information`() = integrationTest {
                // Given
                val symbol = TestFixtures.Symbols.AAPL
        ...
        inner class SummaryDetailModuleSpec {
            ...
            fun `can get stock summary details`() = integrationTest {
                // Given
                val symbol = TestFixtures.Symbols.AAPL
        ...
        inner class FinancialDataModuleSpec {
            ...
            fun `can get financial data`() = integrationTest {
                // Given
                val symbol = TestFixtures.Symbols.AAPL
        ...
        inner class AssetProfileModuleSpec {
            ...
            fun `can get asset profile information`() = integrationTest {
                // Given
                val symbol = TestFixtures.Symbols.AAPL
        ...
        inner class QuoteTypeModuleSpec {
            ...
            fun `can get quote type information`() = integrationTest {
                // Given
                val symbol = TestFixtures.Symbols.AAPL
        ...
        inner class EarningsTrendModuleSpec {
            ...
            fun `can get earnings trend information`() = integrationTest {
                // Given
                val symbol = TestFixtures.Symbols.AAPL
        ...
        inner class MajorHoldersModuleSpec {
            ...
            fun `major holders module may not be supported`() = integrationTest {
                // Given
                val symbol = TestFixtures.Symbols.AAPL
                ...
                assertThat(result.isFailure || result.getOrNull()?.hasModule(QuoteSummaryModule.MAJOR_HOLDERS) == false)
                    .isTrue()
            }
        }
        ...
        inner class TopHoldingsModuleSpec {
            ...
            fun `can get top holdings for ETF`() = integrationTest {
                // Given
                val symbol = "SPY" // S&P 500 ETF
        ...
        inner class MultipleModulesSpec {
            ...
            fun `can get multiple modules at once`() = integrationTest {
                // Given
                val symbol = TestFixtures.Symbols.AAPL
                val modules = setOf(
                    QuoteSummaryModule.PRICE,
