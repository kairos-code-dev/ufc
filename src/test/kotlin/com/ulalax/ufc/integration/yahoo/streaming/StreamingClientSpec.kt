...
class StreamingClientSpec : IntegrationTestBase() {
    ...
    fun `can establish WebSocket connection`() =
        runBlocking {
            // Given
    ...
    fun `can subscribe to single symbol and receive data`() =
        runBlocking {
            // Given
            val config = StreamingClientConfig(enableLogging = true)
            client = StreamingClient.create(config)
    ...
    fun `can subscribe to multiple symbols and receive data`() =
        runBlocking {
            // Given
            val config = StreamingClientConfig(enableLogging = true)
            client = StreamingClient.create(config)
    ...
    fun `can unsubscribe from symbols`() =
        runBlocking {
            // Given
            val config = StreamingClientConfig(enableLogging = true)
            client = StreamingClient.create(config)
    ...
    fun `can unsubscribe all symbols`() =
        runBlocking {
            // Given
            val config = StreamingClientConfig(enableLogging = true)
            client = StreamingClient.create(config)
    ...
    fun `can disconnect and reconnect`() =
        runBlocking {
            // Given
            val config = StreamingClientConfig(enableLogging = true)
            client = StreamingClient.create(config)
    ...
    fun `can receive StreamingQuote data`() =
        runBlocking {
            // Given
            val config = StreamingClientConfig(enableLogging = true)
            client = StreamingClient.create(config)
    ...
    fun `can track connection status through events`() =
        runBlocking {
            // Given
            val config = StreamingClientConfig(enableLogging = true)
            client = StreamingClient.create(config)
    ...
    fun `can receive subscription update events`() =
        runBlocking {
            // Given
            val config = StreamingClientConfig(enableLogging = true)
            client = StreamingClient.create(config)
