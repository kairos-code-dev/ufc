...
class ScreenerSpec : IntegrationTestBase() {
    ...
    inner class PredefinedScreeners {
        ...
        fun `returns day gainers`() = integrationTest {
            // When
        ...
        fun `returns most actives`() = integrationTest {
            // When
        ...
        fun `returns screener by string id`() = integrationTest {
            // When
        ...
        fun `supports custom sorting`() = integrationTest {
            // When - 시가총액 기준 오름차순 정렬
            val result = ufc.yahoo.screener(
    ...
    inner class CustomQueries {
        ...
        fun `filters by market cap greater than 1 billion`() = integrationTest {
        ...
        fun `filters by sector and market cap`() = integrationTest {
        ...
        fun `filters by pe ratio between 0 and 20`() = integrationTest {
            // Given - PER 0-20 사이
            val query = EquityQuery.and(
        ...
        fun `filters by multiple exchanges`() = integrationTest {
    ...
    inner class PaginationAndSorting {
        ...
        fun `supports pagination with offset and size`() = integrationTest {
            // Given
            val query = EquityQuery.gt(EquityField.INTRADAY_MARKET_CAP, 10_000_000_000)

            // When - 첫 번째 페이지
            val page1 = ufc.yahoo.screener(
                query = query,
                sortField = ScreenerSortField.MARKET_CAP,
                sortAsc = false,
                size = 5,
        ...
        fun `supports ascending and descending sort`() = integrationTest {
            // Given
            val query = EquityQuery.gt(EquityField.INTRADAY_MARKET_CAP, 100_000_000_000)

            // When - 내림차순
            val descResult = ufc.yahoo.screener(
                query = query,
                sortField = ScreenerSortField.MARKET_CAP,
                sortAsc = false,
                size = 5
            ...
            // When - 오름차순
            val ascResult = ufc.yahoo.screener(
                query = query,
                sortField = ScreenerSortField.MARKET_CAP,
                sortAsc = true,
                size = 5
    ...
    inner class ErrorHandling {
        ...
        fun `throws exception for invalid size`() = integrationTest {
            // Given
            val query = EquityQuery.gt(EquityField.INTRADAY_MARKET_CAP, 1_000_000_000)
        ...
        fun `throws exception for negative offset`() = integrationTest {
            // Given
            val query = EquityQuery.gt(EquityField.INTRADAY_MARKET_CAP, 1_000_000_000)
        ...
        fun `throws exception for invalid predefined id`() = integrationTest {
            // When/Then
            assertThatThrownBy {
                runBlocking {
    ...
    inner class ResponseStructure {
        ...
        fun `validates screener result structure`() = integrationTest {
            // When
        ...
        fun `stores dynamic fields in additional fields`() = integrationTest {
            // When
            val result = ufc.yahoo.screener(
    ...
    inner class QueryValidation {
        ...
        fun `and operator requires at least 2 operands`() {
            // When/Then
            assertThatThrownBy {
                EquityQuery(ScreenerOperator.AND, listOf(
                    EquityQuery.gt(EquityField.INTRADAY_MARKET_CAP, 1_000_000_000)
                )).validate()
            }.isInstanceOf(IllegalArgumentException::class.java)
        ...
        fun `gt operator requires number value`() {
            // When/Then
            assertThatThrownBy {
                EquityQuery(ScreenerOperator.GT, listOf(
        ...
        fun `btwn operator requires exactly 3 operands`() {
            // When/Then
            assertThatThrownBy {
                EquityQuery(ScreenerOperator.BTWN, listOf(
